...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //Active player may onle be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
        ...
        if(activePlayer === 'O') {
            ...
            disableClick();
            //this fnction wits 1 second before placing the image
            //and enabling click.
...
function checkWinConditions() {
    ...
    // This condtiion checks for tie.  If none of the above conditions register
    ...
    function arrayIncludes(sqaureA, squareB, squareC) {
        ...
        //retruned and our else if condtion executes the drawWinLine function.
...
function audio(audioURL) {
    // We create a new audio object and we pas the path as a paramenter.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        c.moveTo(x1, y1);
        // This metho indicates the end point in our line.
        c.lineTo(x, y);
        ...
        // It was necessaty for the 6, 4, 2 wind condition.
        if (x1 <= x2 && y1 >= y2) {
            if (x < x2) { x += 10; }
            if (y > y2) { y -= 10; }
            if (x >= x2 && y <= y2) { cancelAnimationFrame(animationLoop);}
...
function resetGame() {
    // This for loop iterates through each HTML square elements
    for (let i = 0; i < 9; i++) {
